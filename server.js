...
io.on('connection', (socket) => {
  ...
  socket.on('create-session', (userName) => {
    ...
    socket.emit('session-joined', { sessionId, userName, session });
  });
  ...
  socket.on('join-session', ({ sessionId, userName }) => {
    ...
    if (session.users[userName] !== undefined) {
      socket.emit('error-msg', 'Bu isim zaten kullanılıyor!');
      return;
    }
    ...
    socket.emit('session-joined', { sessionId: session.id, userName, session });
    socket.to(session.id).emit('session-updated', session);
  });
